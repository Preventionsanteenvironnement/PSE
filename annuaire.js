/* 📁 ANNUAIRE CENTRAL & CONFIGURATION - Dépôt PSE
   Mise à jour : Normalisation des identifiants + Mouchard fiable (Firebase v8)
   - Les codes élèves fonctionnent en minuscules ou majuscules (normalisation -> MAJ)
   - Les visites anonymes sont regroupées sous "INV"
*/
...
// Utilitaire : normaliser le code élève (minuscule/majuscule OK)
function normaliserCode(raw) {
  const c = (raw || "").toString().trim();
  return c ? c.toUpperCase() : "INV";
}
...
async function enregistrerVisite(nomPage) {
  try {
    ...
    // S’assurer que Firebase est initialisé (v8)
    if (!firebase.apps.length) firebase.initializeApp(firebaseConfig);
    const db = firebase.firestore();

    // A) Identité (minuscule/majuscule OK)
    const raw = localStorage.getItem("codeEleve");
    const codeActuel = normaliserCode(raw);

    // Optionnel : on stocke la version normalisée (évite ka47/KA47)
    if (raw && raw !== codeActuel) localStorage.setItem("codeEleve", codeActuel);

    // B) Classe via annuaire
    const classeActuelle = ANNUAIRE[codeActuel] || "AUTRE";
    ...
    // E) Type (anonyme/identifié)
    const isAnonymous = (codeActuel === "INV" || classeActuelle === "VISITEUR");
...
console.log("✅ Annuaire chargé. Mouchard prêt (Code normalisé + Firebase v8).");
